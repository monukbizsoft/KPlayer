/*****************************************************************************
 * ArtworkProvider.kt
 * Copyright © 2011-2021 KPlayer authors and VideoLAN
 *
...
import com.kbizsoft.tools.removeFileScheme
import com.kbizsoft.KPlayer.gui.helpers.AudioUtil
import com.kbizsoft.KPlayer.gui.helpers.getBitmapFromDrawable
import com.kbizsoft.KPlayer.media.MediaSessionBrowser
import com.kbizsoft.KPlayer.util.AccessControl
...
private const val TAG = "KPlayer/ArtworkProvider"
private const val MIME_TYPE_IMAGE_WEBP = "image/webp"
private const val ENABLE_TRACING = false

/**
 * This content provider enables callers to retrieve cover artwork cataloged by the KPlayer Medialibrary.
 *
 ...
 * content://com.kbizsoft.KPlayer.artwork/album/<track count>/<id>
 * content://com.kbizsoft.KPlayer.artwork/artist/<track count>/<id>
 * content://com.kbizsoft.KPlayer.artwork/media/<last modified>/<id>
 * content://com.kbizsoft.KPlayer.artwork/play_all/artist/<track count>/<id>
 * content://com.kbizsoft.KPlayer.artwork/play_all/genre/<track count>/<id>
 * content://com.kbizsoft.KPlayer.artwork/play_all/playlist/<CRC32 checksum>/<track count>/<id>
 * content://com.kbizsoft.KPlayer.artwork/shuffle_all/<half day expiration time>/<track count>
 * content://com.kbizsoft.KPlayer.artwork/history/<XOR checksum>/<track count>
 * content://com.kbizsoft.KPlayer.artwork/last_added/<XOR checksum>/<track count>
 *
 * Non-Medialibrary remote cache:
 * content://com.kbizsoft.KPlayer.artwork/remote?path=<encoded uri>
 */
class ArtworkProvider : ContentProvider() {
    ...
    private fun getRemoteImage(ctx: Context, path: String?): ParcelFileDescriptor? {
        ...
        val image = getOrPutImage(path) {
            runBlocking(Dispatchers.IO) {
                ...
                if (bitmap == null) bitmap = ctx.getBitmapFromDrawable(R.drawable.icon, width, width)
    ...
    private fun readEmbeddedArtwork(mw: MediaLibraryItem?, width: Int): Bitmap? {
        if (mw is MediaWrapper && mw.artworkMrl == null && mw.uri != null) {
            ...
            return try {
                val libKplayer = KPlayerInstance.getInstance(ctx)
                val mediaFactory = FactoryManager.getFactory(IMediaFactory.factoryId) as IMediaFactory
                media = mediaFactory.getFromUri(libKplayer, mw.uri).apply { parse() }
