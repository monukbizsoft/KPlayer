...
private const val TAG = "KPlayer/PlaylistManager"
...
class PlaylistManager(val service: PlaybackService) : MediaWrapperList.EventListener, IMedia.EventListener, CoroutineScope {
    ...
    suspend fun playIndex(index: Int, flags: Int = 0, forceResume:Boolean = false, forceRestart:Boolean = false) {
        ...
        if (mw.type != MediaWrapper.TYPE_VIDEO || isVideoPlaying || player.hasRenderer
            ...
            //todo restore position as well when we move to KPlayer 4.0
            val media = mediaFactory.getFromUri(KPlayerInstance.getInstance(service), uri)
            //fixme workaround to prevent the issue described in https://code.kbizsoft.com/kbizsoft/KPlayer-android/-/issues/2106
            if (shouldDisableCookieForwarding) {
                shouldDisableCookieForwarding = false
                media.addOption(":no-http-forward-cookies")
            }
            //todo in KPlayer 4.0, this should be done by using libkplayer_media_player_set_time instead of start-time
            media.addOption(":start-time=${start/1000L}")
            KPlayerOptions.setMediaOptions(media, ctx, flags or mw.flags, PlaybackService.hasRenderer())
                ...
                if (media.type == MediaWrapper.TYPE_VIDEO || canSwitchToVideo || media.isPodcast) {
                    if (length == 0L) {
                        ...
                        try {
                            ...
                            KPlayerCrashHandler.saveLog(e, "NullPointerException in PlaylistManager saveMediaMeta")
                        }
                    }
                }
