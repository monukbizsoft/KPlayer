package com.kbizsoft.KPlayer.media
...
import com.kbizsoft.libkplayer.interfaces.IMediaList
...
class PlayerController(val context: Context) : IKPlayerVout.Callback, MediaPlayer.EventListener, CoroutineScope {
    override val coroutineContext = Dispatchers.Main.immediate + SupervisorJob()

    //    private val exceptionHandler by lazy(LazyThreadSafetyMode.NONE) { CoroutineExceptionHandler { _, _ -> onPlayerError() } }
    private val playerContext by lazy(LazyThreadSafetyMode.NONE) { newSingleThreadContext("KPlayer-player") }
    ...
    fun getVout(): IKPlayerVout? = mediaplayer.kplayerVout
    ...
    internal suspend fun startPlayback(media: IMedia, listener: MediaPlayerEventListener, time: Long) {
        ...
        if (!mediaplayer.isReleased) {
            mediaplayer.setEqualizer(KPlayerOptions.getEqualizerSetFromSettings(context))
    ...
    fun isVideoPlaying() = !mediaplayer.isReleased && mediaplayer.kplayerVout.areViewsAttached()
    ...
    fun getVideoTracks(): Array<out KplayerTrack> = if (!mediaplayer.isReleased && mediaplayer.hasMedia()) mediaplayer.getAllVideoTracks() else emptyArray()

    fun getVideoTrack():String = if (!mediaplayer.isReleased && mediaplayer.hasMedia()) mediaplayer.getSelectedVideoTrack()?.getId() ?: "-1" else "-1"

    fun getCurrentVideoTrack(): KplayerTrack? = if (!mediaplayer.isReleased && mediaplayer.hasMedia()) mediaplayer.getSelectedVideoTrack() else null

    fun getAudioTracksCount() = if (!mediaplayer.isReleased && mediaplayer.hasMedia()) mediaplayer.getAudioTracksCount() else 0

    fun getAudioTracks(): Array<out KplayerTrack>? = if (!mediaplayer.isReleased && mediaplayer.hasMedia()) mediaplayer.getAllAudioTracks() else emptyArray()
    ...
    fun unselectTrackType(trackType: VideoTracksDialog.TrackType) {
        val kplayerTrackType = when(trackType) {
            VideoTracksDialog.TrackType.VIDEO -> 1
            VideoTracksDialog.TrackType.AUDIO -> 0
            VideoTracksDialog.TrackType.SPU -> 2
        }
        if (!mediaplayer.isReleased && mediaplayer.hasMedia()) mediaplayer.unselectTrackType(kplayerTrackType)
    }
    ...
    fun getSpuTracks(): Array<out KplayerTrack>? = mediaplayer.getAllSpuTracks()
    ...
    fun release(player: MediaPlayer = mediaplayer) {
        ...
        if (isVideoPlaying()) player.kplayerVout.detachViews()
        releaseMedia()
    ...
    private fun newMediaPlayer() : MediaPlayer {
        return MediaPlayer(KPlayerInstance.getInstance(context)).apply {
            setAudioDigitalOutputEnabled(KPlayerOptions.isAudioDigitalOutputEnabled(settings))
            KPlayerOptions.getAout(settings)?.let { setAudioOutput(it) }
            setRenderer(PlaybackService.renderer.value)
            this.kplayerVout.addCallback(this@PlayerController)
        }
    }
