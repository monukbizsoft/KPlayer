...
import com.kbizsoft.libkplayer.util.MediaBrowser.EventListener
...
const val TAG = "KPlayer/BrowserProvider"
...
abstract class BrowserProvider(val context: Context, val dataset: LiveDataset<MediaLibraryItem>, val url: String?, var sort:Int, var desc:Boolean) : CoroutineScope, HeaderProvider() {
    ...
    protected open fun initBrowser() {
        if (mediabrowser == null) {
            registerCreator { MediaBrowser(KPlayerInstance.getInstance(context), null, browserHandler) }
    ...
    companion object : DependencyProvider<BrowserProvider>() {
        private val browserHandler by lazy {
            val handlerThread = HandlerThread("KPlayer-provider", Process.THREAD_PRIORITY_DEFAULT + Process.THREAD_PRIORITY_LESS_FAVORABLE)
            handlerThread.start()
