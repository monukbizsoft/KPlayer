/*****************************************************************************
 * PlaybackService.kt
 * Copyright © 2011-2018 KPlayer authors and VideoLAN
 *
...
import com.kbizsoft.libkplayer.FactoryManager
...
import com.kbizsoft.libkplayer.interfaces.IMediaFactory
...
import com.kbizsoft.resources.util.KPlayerCrashHandler
import com.kbizsoft.resources.util.getFromMl
import com.kbizsoft.resources.util.launchForeground
import com.kbizsoft.resources.util.registerReceiverCompat
import com.kbizsoft.resources.util.startForegroundCompat
import com.kbizsoft.tools.*
import com.kbizsoft.KPlayer.gui.dialogs.VideoTracksDialog
import com.kbizsoft.KPlayer.gui.dialogs.adapters.KplayerTrack
import com.kbizsoft.KPlayer.gui.helpers.AudioUtil
import com.kbizsoft.KPlayer.gui.helpers.NotificationHelper
import com.kbizsoft.KPlayer.gui.helpers.getBitmapFromDrawable
import com.kbizsoft.KPlayer.gui.video.PopupManager
import com.kbizsoft.KPlayer.gui.video.VideoPlayerActivity
import com.kbizsoft.KPlayer.media.*
import com.kbizsoft.KPlayer.util.*
import com.kbizsoft.KPlayer.widget.MiniPlayerAppWidgetProvider
import com.kbizsoft.KPlayer.widget.KPlayerAppWidgetProvider
...
private const val TAG = "KPlayer/PlaybackService"
...
class PlaybackService : MediaBrowserServiceCompat(), LifecycleOwner, CoroutineScope, SchedulerCallback {
    ...
    private val audioFocusHelper by lazy { KPlayerAudioFocusHelper(this) }
    ...
    private val receiver = object : BroadcastReceiver() {
        ...
        override fun onReceive(context: Context, intent: Intent) {
            ...
            when (action) {
                ...
                KPlayerAppWidgetProvider.ACTION_WIDGET_INIT -> updateWidget()
                KPlayerAppWidgetProvider.ACTION_WIDGET_ENABLED, KPlayerAppWidgetProvider.ACTION_WIDGET_DISABLED -> updateHasWidget()
    ...
    suspend fun trackInfo(): String? {
        ...
        val media = withContext(Dispatchers.IO) {
            val libKplayer = KPlayerInstance.getInstance(this@PlaybackService)
            mediaFactory.getFromUri(libKplayer, mediaWrapper.uri).apply { parse() }
        }
    ...
    val audioTracks: Array<out KplayerTrack>?
        @MainThread
    ...
    val videoTracks: Array<out KplayerTrack>?
        @MainThread
        get() = playlistManager.player.getVideoTracks()

    val currentVideoTrack: KplayerTrack?
        @MainThread
    ...
    val spuTracks: Array<out KplayerTrack>?
        @MainThread
    ...
    override fun onCreate() {
        ...
        val filter = IntentFilter().apply {
            ...
            addAction(KPlayerAppWidgetProvider.ACTION_WIDGET_INIT)
            addAction(KPlayerAppWidgetProvider.ACTION_WIDGET_ENABLED)
            addAction(KPlayerAppWidgetProvider.ACTION_WIDGET_DISABLED)
    ...
    private fun updateHasWidget() {
        ...
        widget = when {
            manager.getAppWidgetIds(ComponentName(this, KPlayerAppWidgetProviderWhite::class.java)).isNotEmpty() -> 1
            manager.getAppWidgetIds(ComponentName(this, KPlayerAppWidgetProviderBlack::class.java)).isNotEmpty() -> 2
    ...
    private fun sendStartSessionIdIntent() {
        val sessionId = KPlayerOptions.audiotrackSessionId
    ...
    private fun initMediaSession() {
        ...
        val playbackState = PlaybackStateCompat.Builder()
                ...
                .build()
        mediaSession = MediaSessionCompat(this, "KPlayer", mbrName, mbrIntent).apply {
    ...
    private fun sendWidgetBroadcast(intent: Intent) {
        intent.component = ComponentName(this@PlaybackService, if (widget == 1) KPlayerAppWidgetProviderWhite::class.java else if (widget == 3) MiniPlayerAppWidgetProvider::class.java else KPlayerAppWidgetProviderBlack::class.java)
        sendBroadcast(intent)
    }

    fun updateWidgetState() {
        val widgetIntents = arrayOf(Intent(KPlayerAppWidgetProvider.ACTION_WIDGET_UPDATE), Intent(MiniPlayerAppWidgetProvider.ACTION_WIDGET_UPDATE))
    ...
    private fun broadcastMetadata() {
        ...
        if (lifecycleScope.isActive) lifecycleScope.launch(Dispatchers.Default) {
            sendBroadcast(Intent("com.android.music.metachanged")
                    ...
                    .putExtra("package", "com.kbizsoft.KPlayer")
                    .apply {
        ...
        artworkMap = HashMap<String, Uri>().also {
            ...
            for (media in playlistManager.getMediaList()) {
                try {
                    ...
                    KPlayerCrashHandler.saveLog(e, "NullPointerException in PlaybackService updateMediaQueue")
                }
            }
     ...
     * @param flags LibKPlayer.MEDIA_* flags
     */
    @JvmOverloads
