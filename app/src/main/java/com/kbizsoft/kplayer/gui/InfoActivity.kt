...
import com.kbizsoft.libkplayer.util.Extensions
...
import com.kbizsoft.resources.TAG_ITEM
...
import com.kbizsoft.KPlayer.databinding.InfoActivityBinding
import com.kbizsoft.KPlayer.getAllTracks
import com.kbizsoft.KPlayer.gui.browser.PathAdapter
import com.kbizsoft.KPlayer.gui.browser.PathAdapterListener
import com.kbizsoft.KPlayer.gui.helpers.AudioUtil
import com.kbizsoft.KPlayer.gui.helpers.FloatingActionButtonBehavior
import com.kbizsoft.KPlayer.gui.helpers.MedialibraryUtils
import com.kbizsoft.KPlayer.gui.video.MediaInfoAdapter
import com.kbizsoft.KPlayer.gui.view.KPlayerDividerItemDecoration
import com.kbizsoft.KPlayer.media.MediaUtils
import com.kbizsoft.KPlayer.util.*
import com.kbizsoft.KPlayer.viewmodels.browser.IPathOperationDelegate
import com.kbizsoft.KPlayer.viewmodels.browser.PathOperationDelegate
import java.io.File
import java.util.*

private const val TAG = "KPlayer/InfoActivity"
            ...
            item.itemType == MediaLibraryItem.TYPE_MEDIA -> {
                ...
                if (isSchemeSupported(media.uri?.scheme)) {
                    ...
                    if (binding.ariane.itemDecorationCount == 0) {
                        binding.ariane.addItemDecoration(KPlayerDividerItemDecoration(this@InfoActivity, DividerItemDecoration.HORIZONTAL, ContextCompat.getDrawable(this@InfoActivity, R.drawable.ic_divider)!!))
                    }
...
class InfoModel : ViewModel() {
    ...
    fun parseTracks(context: Context, mw: MediaWrapper) = viewModelScope.launch {
        val media = withContext(Dispatchers.IO) {
            val libKplayer = KPlayerInstance.getInstance(context)
            mediaFactory.getFromUri(libKplayer, mw.uri).apply { parse() }
        }
