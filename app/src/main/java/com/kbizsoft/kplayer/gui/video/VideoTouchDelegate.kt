...
class VideoTouchDelegate(private val player: VideoPlayerActivity,
    ...
    companion object {
        private const val TAG = "KPlayer/VideoTouchDelegate"
                ...
                when (event.action) {
                    ...
                    MotionEvent.ACTION_UP -> {
                        ...
                        if (numberOfTaps > 1 && !player.isLocked) {
                            ...
                            when {
                                (touchControls and TOUCH_FLAG_DOUBLE_TAP_SEEK != 0) && event.x < range / 4f -> seekDelta(-com.kbizsoft.tools.Settings.videoDoubleTapJumpDelay * 1000)
                                (touchControls and TOUCH_FLAG_DOUBLE_TAP_SEEK != 0) && event.x > range * 0.75 -> seekDelta(com.kbizsoft.tools.Settings.videoDoubleTapJumpDelay * 1000)
