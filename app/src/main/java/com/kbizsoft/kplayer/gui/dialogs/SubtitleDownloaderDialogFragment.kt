...
import com.kbizsoft.KPlayer.databinding.SubtitleDownloaderDialogBinding
import com.kbizsoft.KPlayer.gui.helpers.UiTools
import com.kbizsoft.KPlayer.gui.helpers.UiTools.deleteSubtitleDialog
import com.kbizsoft.KPlayer.gui.view.OnItemSelectListener
import com.kbizsoft.KPlayer.media.MediaUtils
import com.kbizsoft.KPlayer.util.KPlayerDownloadManager
import com.kbizsoft.KPlayer.viewmodels.SubtitlesModel

private const val MEDIA_PATHS = "MEDIA_PATHS"
private const val MEDIA_NAMES = "MEDIA_NAMES"

class SubtitleDownloaderDialogFragment : KPlayerBottomSheetDialogFragment() {
    ...
    private val listEventActor = lifecycleScope.actor<SubtitleEvent> {
        for (subtitleEvent in channel) if (isActive) when (subtitleEvent) {
            is SubtitleClick -> when (subtitleEvent.item.state) {
                State.NotDownloaded -> KPlayerDownloadManager.download(requireActivity(), subtitleEvent.item)
