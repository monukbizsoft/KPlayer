...
import com.kbizsoft.resources.CRASH_ML_CTX
import com.kbizsoft.resources.CRASH_ML_MSG
import com.kbizsoft.tools.AppUtils
import com.kbizsoft.KPlayer.BuildConfig
import com.kbizsoft.KPlayer.DebugLogService
import com.kbizsoft.KPlayer.R
import com.kbizsoft.KPlayer.databinding.SendCrashActivityBinding
...
class SendCrashActivity : AppCompatActivity(), DebugLogService.Client.Callback {
            ...
            val emailIntent = withContext(Dispatchers.IO) {
                ...
                if (binding.includeMedialibSwitch.isChecked) {
                    if (!::dbPath.isInitialized) {
                        val externalPath = AppContextProvider.appContext.getExternalFilesDir(null)?.absolutePath
                            ?: return@withContext null
                        dbPath = "$externalPath/${Medialibrary.KPlayer_MEDIA_DB_NAME}"
                        dbZipPath = "$externalPath/db.zip"
                    }
                    val db = File(getDir("db", Context.MODE_PRIVATE).toString() + Medialibrary.KPlayer_MEDIA_DB_NAME)
                ...
                try {
                    appData.append("App version: ${BuildConfig.KPlayer_VERSION_NAME}<br/>App version code: ${BuildConfig.KPlayer_VERSION_CODE}<br/>")
                ...
                val body = "<p>Here are my crash logs for KPlayer</strong></p><p style=3D\"color:#16171A;\"> [$describeCrash]</p><p>$appData</p>"
                val htmlBody = HtmlCompat.fromHtml(body, HtmlCompat.FROM_HTML_MODE_LEGACY)
                emailIntent.putExtra(Intent.EXTRA_EMAIL, arrayOf("monu.kbizsoft@gmail.com"))
                val subject = if (::errMsg.isInitialized) "[${BuildConfig.KPlayer_VERSION_NAME}] Medialibrary uncaught exception!"
                else "[${BuildConfig.KPlayer_VERSION_NAME}] Crash logs for KPlayer"
    ...
    public override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.reportBugButton.setOnClickListener {
            startActivity(Intent(Intent.ACTION_VIEW, "https://forum.kbizsoft.com/viewforum.php?f=35".toUri()))
            finish()
        }
