...
import com.kbizsoft.KPlayer.databinding.PlayerOptionItemBinding
import com.kbizsoft.KPlayer.gui.AudioPlayerContainerActivity
import com.kbizsoft.KPlayer.gui.BaseActivity
...
import com.kbizsoft.KPlayer.media.PlayerController
import com.kbizsoft.KPlayer.util.getScreenHeight
...
class PlayerOptionsDelegate(val activity: FragmentActivity, val service: PlaybackService, private val showABReapeat:Boolean = true) : LifecycleObserver {
    ...
    private fun showFragment(id: Long) {
        ...
        if (newFragment is KPlayerBottomSheetDialogFragment && activity is VideoPlayerActivity)
    ...
    private fun togglePassthrough() {
        val enabled = !KPlayerOptions.isAudioDigitalOutputEnabled(settings)
        if (service.setAudioDigitalOutputEnabled(enabled)) {
            ...
            KPlayerOptions.setAudioDigitalOutputEnabled(settings, enabled)
