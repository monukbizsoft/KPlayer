...
import com.kbizsoft.tools.setGone
import com.kbizsoft.tools.setVisible
import com.kbizsoft.KPlayer.PlaybackService
import com.kbizsoft.KPlayer.R
import com.kbizsoft.KPlayer.databinding.PlayerOverlayTracksBinding
import com.kbizsoft.KPlayer.getDisableTrack
import com.kbizsoft.KPlayer.gui.dialogs.adapters.TrackAdapter
...
class VideoTracksDialog : KPlayerBottomSheetDialogFragment() {
    ...
    private fun onServiceChanged(service: PlaybackService?) {
        service?.let { playbackService ->
            if ((isKPlayer4() && playbackService.videoTracksCount < 2) || (!isKPlayer4() && playbackService.videoTracksCount <= 2)) {
    ...
    private fun generateTrackList(trackList: Array<out KplayerTrack>) = if (isKPlayer4()) {
        val tempTracks = ArrayList<KplayerTrack>()
        tempTracks.add(getDisableTrack(requireActivity()))
        trackList.forEach {
            tempTracks.add(it)
        }
        tempTracks.toList().toTypedArray()
    } else trackList as Array<KplayerTrack>
