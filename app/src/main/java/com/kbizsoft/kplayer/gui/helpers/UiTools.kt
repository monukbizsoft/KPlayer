...
import com.kbizsoft.medialibrary.interfaces.media.*
...
import com.kbizsoft.KPlayer.BuildConfig.KPlayer_VERSION_NAME
import com.kbizsoft.KPlayer.MediaParsingService
import com.kbizsoft.KPlayer.R
import com.kbizsoft.KPlayer.StartActivity
import com.kbizsoft.KPlayer.gui.*
import com.kbizsoft.KPlayer.gui.browser.MediaBrowserFragment
import com.kbizsoft.KPlayer.gui.dialogs.*
import com.kbizsoft.KPlayer.gui.helpers.BitmapUtil.vectorToBitmap
import com.kbizsoft.KPlayer.gui.helpers.hf.PinCodeDelegate
import com.kbizsoft.KPlayer.gui.helpers.hf.checkPIN
import com.kbizsoft.KPlayer.gui.preferences.PreferencesActivity
import com.kbizsoft.KPlayer.media.MediaUtils
import com.kbizsoft.KPlayer.media.getAll
...
import com.kbizsoft.KPlayer.util.SchedulerCallback
import com.kbizsoft.KPlayer.util.ThumbnailsProvider
import com.kbizsoft.KPlayer.util.openLinkIfPossible
import kotlin.math.min

object UiTools {
    var currentNightMode: Int = 0
    private const val TAG = "KPlayer/UiTools"
    ...
    fun fillAboutView(activity: FragmentActivity, v: View) {
        ...
        appVersion.text = KPlayer_VERSION_NAME
        ...
        v.findViewById<View>(R.id.about_website_container).setOnClickListener {
            activity.openLinkIfPossible("https://www.kbizsoft.com/KPlayer/")
        }
        v.findViewById<View>(R.id.about_forum_container).setOnClickListener {
            activity.openLinkIfPossible("https://forum.kbizsoft.com/viewforum.php?f=35")
        }
        v.findViewById<View>(R.id.about_sources_container).setOnClickListener {
            activity.openLinkIfPossible("https://code.kbizsoft.com/kbizsoft/KPlayer-android")
        }
        ...
        v.findViewById<View>(R.id.about_kplayer_card).setOnClickListener {
           var licenseText = ""
            activity.lifecycleScope.launchWhenStarted {
                licenseText = AppContextProvider.appResources.openRawResource(R.raw.kplayer_license).bufferedReader().use {
                   it.readText()
               }
            }
...
//        KPlayerBilling.getInstance(activity.application).addStatusListener {
...
//        if (KPlayerBilling.getInstance(activity.application).status == BillingStatus.FAILURE ||  KPlayerBilling.getInstance(activity.application).skuDetails.isEmpty()) donationsButton.setGone() else donationsButton.setVisible()
//    }
    ...
    suspend fun FragmentActivity.createShortcut(mediaLibraryItem: MediaLibraryItem) {
        ...
        withContext(Dispatchers.IO) {
            ...
            val iconCompat = IconCompat.createWithAdaptiveBitmap(iconBitmap.centerCrop(size, size)
                    ?: vectorToBitmap(context, R.drawable.icon, 48.dp, 48.dp))
            ...
            val pinShortcutInfo = ShortcutInfoCompat.Builder(context, mediaLibraryItem.id.toString())
                    ...
                    .setIntent(Intent(context, StartActivity::class.java).apply { action = "KPlayer.mediashortcut:$actionType:${mediaLibraryItem.id}" })
                    .setIcon(iconCompat)
                    .build()
    ...
    fun FragmentActivity.showDonations() {
        if (!isStarted()) return
//        val videoTracksDialog = KPlayerBillingDialog()
//        videoTracksDialog.show(supportFragmentManager, "fragment_donations")
    ...
    fun restartDialog(context: Context) {
        AlertDialog.Builder(context)
                .setTitle(context.resources.getString(R.string.restart_kplayer))
...
fun BaseActivity.applyTheme() {
    ...
    if (Settings.showTvUi) {
        ...
        setTheme(R.style.Theme_KPlayer_Black)
        return
    }
