 ...
 * BenchActivity.java
 *
 * Copyright © 2011-2014 KPlayer authors and VideoLAN
 *
...
package com.kbizsoft.KPlayer.gui.video.benchmark
...
import com.kbizsoft.KPlayer.media.PlaylistManager
import java.io.*
 ...
 * - PLAYBACK:
 * The class just plays the video, sending back statistics to KPlayerBenchmark.
 * - SCREENSHOTS:
 ...
 * Once the buffering is finished, a broadcast is sent to com.kbizsoft.kplayerbenchmark's service
 * to take a screenshot. Screenshots are taken in KPlayerBenchmark because, as of Android 10,
 ...
 * on KPlayer-android. By using this API in a service in KPlayerBenchmark, the user is only asked permission
 * once, when the user starts the benchmark.
 * KPlayerBenchmark then uses a broadcast to ask KPlayer-android to resume the sample to the next
...
class BenchActivity : ShallowVideoPlayer() {
    ...
    /* set to true when KPlayer crashes */
    private var hasKPlayerFailed = false
    ...
    /* To avoid storing benchmark samples in the user's KPlayer history, the user's preference is
    *  saved, to be restored at the end of the test */
    ...
    /* File in which KPlayer will store logs in case of a crash or freeze */
    private var stacktraceFile: String? = null

    /* Receive continue benchmark action from KPlayerBenchmark after having taken screenshots */
    ...
    *  extend the length of the benchmark*/
     ...
     * To avoid stopping the benchmark, a timeout is set to stop KPlayer
    ...
    private fun setTimeout() {
        if (isSetup) {
            ...
            timeOut = Runnable {
                Log.e(TAG, "KPlayer Seek Froze")
                errorFinish("KPlayer Seek Froze")
            }
     ...
     * trigger screenshot in com.kbizsoft.kplayerbenchmark's service
     *
        ...
        when (event.type) {
            ...
            MediaPlayer.Event.TimeChanged -> {
                ...
                if (!isScreenshot && !isSpeed && timeLimit > 0 && event.timeChanged > timeLimit) {
                    Log.i(TAG, "onMediaPlayerEvent: closing KPlayer-android after time limit reached")
            ...
            MediaPlayer.Event.PositionChanged -> {
                ...
                if (!isScreenshot) {
                    when {
                        ...
                        positionCounter > 50 -> errorFinish("KPlayer Playback Froze")
            ...
            MediaPlayer.Event.Buffering -> if (event.buffering == 100f) {
                ...
                if (isScreenshot && isSetup && screenshotCount < screenshotsTimestamp!!.size && isSeeking) {
                    ...
                    /* Broadcast to trigger screenshot in KPlayerBenchmark */
     ...
     * if the resultCode is different from RESULT_OK, boolean mKPlayerFailed is set to true,
    ...
    override fun exit(resultCode: Int) {
        if (resultCode != Activity.RESULT_OK) {
            hasKPlayerFailed = true
        }
        super.exit(resultCode)
    }

    /**
     * To be called when the error is big enough to return to KPlayerBenchmark
     * @param resultString error description for display in KPlayerBenchmark
     */
    ...
    /**
     * Method reading KPlayer-android logs so that the benchmark can get the cause
     * of the crash / freeze
     */
    ...
    /**
     * Method analysing KPlayer logs to find warnings,
     * and report them to KPlayerBenchmark
     */
    ...
    companion object {
        ...
        private const val ACTION_TRIGGER_SCREENSHOT = "com.kbizsoft.kplayerbenchmark.TRIGGER_SCREENSHOT"
        private const val ACTION_CONTINUE_BENCHMARK = "com.kbizsoft.KPlayer.gui.video.benchmark.CONTINUE_BENCHMARK"

        private const val SPEED_TEST_ITERATION_LIMIT = 5

        private const val TAG = "KPlayerBenchmark"
