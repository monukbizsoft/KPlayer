 ...
 *****************************************************************************
 * Copyright © 2013-2019 KPlayer authors and VideoLAN
 *
 ...
 *****************************************************************************/
package com.kbizsoft.KPlayer.gui.audio
...
import com.kbizsoft.resources.KPlayerOptions
...
import com.kbizsoft.KPlayer.databinding.EqualizerBinding
import com.kbizsoft.KPlayer.gui.dialogs.KPlayerBottomSheetDialogFragment
import com.kbizsoft.KPlayer.gui.helpers.UiTools
import com.kbizsoft.KPlayer.gui.view.EqualizerBar
import com.kbizsoft.KPlayer.interfaces.OnEqualizerBarChangeListener
import kotlin.math.abs
import kotlin.math.roundToInt

class EqualizerFragment : KPlayerBottomSheetDialogFragment(), Slider.OnChangeListener {
    ...
    private fun fillViews() = lifecycleScope.launch(start = CoroutineStart.UNDISPATCHED) {
        ...
        if (bandCount == -1) bandCount = withContext(Dispatchers.IO) {
            KPlayerInstance.getInstance(requireContext())
        ...
        equalizer = KPlayerOptions.getEqualizerSetFromSettings(requireActivity(), true)!!

        // on/off
        binding.equalizerButton.isChecked = KPlayerOptions.getEqualizerEnabledState(requireActivity())
        ...
        binding.equalizerPresets.post {
            ...
            val pos = allSets.indexOf(KPlayerOptions.getEqualizerNameFromSettings(activity))
            state.update(pos, KPlayerOptions.getEqualizerSavedState(activity))
    ...
    override fun onPause() {
        ...
        if (binding.equalizerButton.isChecked) {
            ...
            if (::equalizer.isInitialized) KPlayerOptions.saveEqualizerInSettings(requireActivity(), equalizer, allSets[pos], true, state.saved)
        } else {
            KPlayerOptions.saveEqualizerInSettings(requireActivity(), MediaPlayer.Equalizer.createFromPreset(0), allSets[0], enabled = false, saved = true)
        }
    }
    ...
    private fun createSaveCustomSetDialog(positionToSave: Int, displayedByUser: Boolean, onPause: Boolean) {
        ...
        val saveEqualizer = AlertDialog.Builder(requireActivity())
                ...
                .setNegativeButton(R.string.do_not_save) { _, _ ->
                    if (onPause)
                        KPlayerOptions.saveEqualizerInSettings(AppContextProvider.appContext, equalizer, allSets[positionToSave], binding.equalizerButton.isChecked, false)
                }
                .setOnCancelListener {
                    if (onPause)
                        KPlayerOptions.saveEqualizerInSettings(AppContextProvider.appContext, equalizer, allSets[positionToSave], binding.equalizerButton.isChecked, false)
    ...
    private fun save(ctx: Context, input: EditText, oldName: String, temporarySet: MediaPlayer.Equalizer, onPause: Boolean, displayedByUser: Boolean, positionToSave: Int, saveEqualizer: AlertDialog) {
        if (input.error == null) {
            val newName = input.text.toString()
            KPlayerOptions.saveCustomSet(ctx, temporarySet, newName)
            if (onPause) {
                if (binding.equalizerButton.isChecked)
                    KPlayerOptions.saveEqualizerInSettings(ctx, temporarySet, newName, enabled = true, saved = true)
            } else {
                if (newName == oldName) {
                    if (displayedByUser) {
    ...
    private fun createDeleteCustomSetSnacker() {
        ...
        if (getEqualizerType(oldPos) == TYPE_CUSTOM) {
            ...
            val savedEqualizerSet = KPlayerOptions.getCustomSet(requireActivity(), oldName) ?: return
            KPlayerOptions.deleteCustomSet(requireActivity(), oldName)
            ...
            UiTools.snackerWithCancel(requireActivity(), message, action = {}) {
                KPlayerOptions.saveCustomSet(requireActivity(), savedEqualizerSet, oldName)
        ...
        if (updateAlreadyHandled) {
            ...
            when {
                ...
                getEqualizerType(pos) == TYPE_CUSTOM -> {
                    equalizer = KPlayerOptions.getCustomSet(requireActivity(), allSets[pos])!!
                    state.update(pos, true)
                }
    ...
    companion object {
        ...
        const val TAG = "KPlayer/EqualizerFragment"
