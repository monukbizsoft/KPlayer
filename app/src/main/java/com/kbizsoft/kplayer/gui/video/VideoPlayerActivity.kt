 ...
 *
 * Copyright © 2011-2017 KPlayer authors and VideoLAN
 *
...
import com.kbizsoft.libkplayer.util.DisplayManager
import com.kbizsoft.libkplayer.util.KPlayerVideoLayout
...
import com.kbizsoft.resources.util.parcelableList
import com.kbizsoft.tools.*
import com.kbizsoft.KPlayer.*
import com.kbizsoft.KPlayer.BuildConfig
import com.kbizsoft.KPlayer.R
import com.kbizsoft.KPlayer.gui.DialogActivity
import com.kbizsoft.KPlayer.gui.audio.EqualizerFragment
import com.kbizsoft.KPlayer.gui.audio.PlaylistAdapter
import com.kbizsoft.KPlayer.gui.browser.EXTRA_MRL
import com.kbizsoft.KPlayer.gui.dialogs.PlaybackSpeedDialog
import com.kbizsoft.KPlayer.gui.dialogs.RenderersDialog
import com.kbizsoft.KPlayer.gui.dialogs.SleepTimerDialog
...
import com.kbizsoft.KPlayer.gui.helpers.hf.StoragePermissionsDelegate
import com.kbizsoft.KPlayer.interfaces.IPlaybackSettingsController
import com.kbizsoft.KPlayer.media.NO_LENGTH_PROGRESS_MAX
import com.kbizsoft.KPlayer.media.VideoResumeStatus
import com.kbizsoft.KPlayer.media.WaitConfirmation
import com.kbizsoft.KPlayer.repository.ExternalSubRepository
import com.kbizsoft.KPlayer.repository.SlaveRepository
import com.kbizsoft.KPlayer.util.*
import com.kbizsoft.KPlayer.util.FileUtils
import com.kbizsoft.KPlayer.util.FileUtils.getUri
import com.kbizsoft.KPlayer.viewmodels.BookmarkModel
import com.kbizsoft.KPlayer.viewmodels.PlaylistModel
import java.io.File
...
open class VideoPlayerActivity : AppCompatActivity(), PlaybackService.Callback, PlaylistAdapter.IPlayer, OnClickListener, OnLongClickListener, StoragePermissionsDelegate.CustomActionController, TextWatcher, IDialogManager, KeycodeListener {
    ...
    var videoLayout: KPlayerVideoLayout? = null
    ...
    private val addedExternalSubs = ArrayList<com.kbizsoft.KPlayer.mediadb.models.ExternalSub>()
    private var downloadedSubtitleLiveData: LiveData<List<com.kbizsoft.KPlayer.mediadb.models.ExternalSub>>? = null
    ...
    private val downloadedSubtitleObserver = Observer<List<com.kbizsoft.KPlayer.mediadb.models.ExternalSub>> { externalSubs ->
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        Log.e("kbtestMob", "VideoPlayerActivity");
    ...
    private fun startPlayback() {
        ...
        service?.run {
            ...
            val kplayerVout = vout
            if (kplayerVout != null && kplayerVout.areViewsAttached()) {
                if (isPlayingPopup) {
                    stop(video = true)
                } else
            ...
            withContext(Dispatchers.IO) {
                videoLayout?.findViewById<FrameLayout>(R.id.player_surface_frame)?.let {
                    ...
                    surfaceView?.let { surface ->
                        ...
                        val dst = File(AndroidDevices.MediaFolders.EXTERNAL_PUBLIC_SCREENSHOTS_URI_DIRECTORY.path + "/kplayer_${simpleDateFormat.format(Date())}.jpg")
                        ...
                        if (event.esChangedType == IMedia.Track.Type.Audio) {
                                ...
                                if (!preferredAudioLang.isNullOrEmpty()) {
                                    ...
                                    service.audioTracks?.iterator()?.let { audioTracks ->
                                        while (audioTracks.hasNext()) {
                                            ...
                                            if (LocaleUtil.getLocaleFromKPlayer(realTrack?.language
                                                            ?: "") == preferredAudioLang) {
                            ...
                            lifecycleScope.launch(Dispatchers.IO) {
                                ...
                                if (addNextTrack) {
                                    ...
                                    if ((tracks as Array<KplayerTrack>).isNotEmpty()) service.setSpuTrack(tracks[tracks.size - 1].getId())
                                    addNextTrack = false
    ...
    private fun handleVout(voutCount: Int) {
        ...
        if (displayManager.isPrimary && kplayerVout.areViewsAttached() && voutCount == 0) {
    ...
    protected open fun loadMedia(fromStart: Boolean = false, forceUsingNew:Boolean = false) {
         ...
         * But onResume will also be called, even if KPlayer-android is still in
         * the background.
    ...
    companion object {
        ...
        private const val TAG = "KPlayer/VideoPlayerActivity"
