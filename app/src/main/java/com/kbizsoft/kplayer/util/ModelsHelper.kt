...
import com.kbizsoft.medialibrary.interfaces.Medialibrary.NbMedia
import com.kbizsoft.medialibrary.interfaces.Medialibrary.SORT_ALBUM
import com.kbizsoft.medialibrary.interfaces.Medialibrary.SORT_ALPHA
import com.kbizsoft.medialibrary.interfaces.Medialibrary.SORT_ARTIST
import com.kbizsoft.medialibrary.interfaces.Medialibrary.SORT_DEFAULT
import com.kbizsoft.medialibrary.interfaces.Medialibrary.SORT_DURATION
import com.kbizsoft.medialibrary.interfaces.Medialibrary.SORT_FILENAME
import com.kbizsoft.medialibrary.interfaces.Medialibrary.SORT_FILESIZE
import com.kbizsoft.medialibrary.interfaces.Medialibrary.SORT_INSERTIONDATE
import com.kbizsoft.medialibrary.interfaces.Medialibrary.SORT_LASTMODIFICATIONDATE
import com.kbizsoft.medialibrary.interfaces.Medialibrary.SORT_PLAYCOUNT
import com.kbizsoft.medialibrary.interfaces.Medialibrary.SORT_RELEASEDATE
import com.kbizsoft.medialibrary.interfaces.Medialibrary.TrackId
...
import com.kbizsoft.medialibrary.media.Storage
import com.kbizsoft.resources.util.getLength
import com.kbizsoft.resources.util.getTimeCategory
import com.kbizsoft.resources.util.getTimeCategoryString
import com.kbizsoft.resources.util.getYear
import com.kbizsoft.resources.util.isSpecialItem
...
fun getFilenameAscComp(): Comparator<MediaLibraryItem> = Comparator<MediaLibraryItem> { item1, item2 ->
    val type1 = (item1 as? MediaWrapper)?.type
    val type2 = (item2 as? MediaWrapper)?.type
    if (type1 == MediaWrapper.TYPE_DIR && type2 != MediaWrapper.TYPE_DIR) return@Comparator -1
    else if (type1 != MediaWrapper.TYPE_DIR && type2 == MediaWrapper.TYPE_DIR) return@Comparator 1
    val filename1 = (item1 as? MediaWrapper)?.fileName?.removeExtension() ?: (item1 as? Storage)?.title
    val filename2 = (item2 as? MediaWrapper)?.fileName?.removeExtension() ?: (item2 as? Storage)?.title
    naturalOrder<String>().compare(filename1?.lowercase(Locale.getDefault()), filename2?.lowercase(Locale.getDefault()))
}

fun getFilenameDescComp(): Comparator<MediaLibraryItem> = Comparator<MediaLibraryItem> { item1, item2 ->
    val type1 = (item1 as? MediaWrapper)?.type
    val type2 = (item2 as? MediaWrapper)?.type
    if (type1 == MediaWrapper.TYPE_DIR && type2 != MediaWrapper.TYPE_DIR) return@Comparator -1
    else if (type1 != MediaWrapper.TYPE_DIR && type2 == MediaWrapper.TYPE_DIR) return@Comparator 1
    val filename1 = (item1 as? MediaWrapper)?.fileName?.removeExtension() ?: (item1 as? Storage)?.title
    val filename2 = (item2 as? MediaWrapper)?.fileName?.removeExtension() ?: (item2 as? Storage)?.title
    naturalOrder<String>().compare(filename2?.lowercase(Locale.getDefault()), filename1?.lowercase(Locale.getDefault()))
}

/**
 * Properly remove the extension part of a filename by not removing it if it leaves an empty string
 *
