...
import com.kbizsoft.resources.util.getPackageInfoCompat
import com.kbizsoft.KPlayer.R
import java.security.MessageDigest
import java.security.NoSuchAlgorithmException

private const val TAG = "KPlayer/AccessControl"
    ...
    fun logCaller(callingUid: Int, clientPackageName: String? = null) {
        ...
        when {
            callingUid == Process.myUid() -> {
                Log.i(TAG, "Known access from self (${ctx.packageName}) to KPlayer")
                callingUidChecked[callingUid] = AuthEntry(true, ctx.packageName, "KPlayer UID")
                return
            }
            callingUid == Process.SYSTEM_UID -> {
                Log.i(TAG, "Known access from system to KPlayer")
            ...
            callingPackage != null -> {
                ...
                if (callingSignature == platformSignature) {
                    Log.i(TAG, "Known access from Android platform ($callingPackage) to KPlayer")
                ...
                certs?.keys?.forEach { key ->
                    if (callingSignature == key) {
                        Log.i(TAG, "Known access from ${certs.name} ($callingPackage) to KPlayer")
                        callingUidChecked[callingUid] = AuthEntry(true, callingPackage, "Known App Signature")
                        return
                    }
                }
                Log.i(TAG, "Unknown access from signature $callingSignature ($callingPackage) to KPlayer")
